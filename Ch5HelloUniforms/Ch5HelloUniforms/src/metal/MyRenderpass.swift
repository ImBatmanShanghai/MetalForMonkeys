...
class MyRenderpass: NSObject {
    ...
    func makeEncoderToCommandBuffer(_ commandBuffer: MTLCommandBuffer,
        ...
        // 3.1 attach renderpass encoder with: input data of vertex attribute
        encoder?.setVertexBuffer(mesh.vertexBuffer!.buffer, offset: 0, index: 0)
        
        // 3.2 attach renderpass encoder with: input data of uniforms
        // ch5.change.3 refresh uniform buffer, and set uniforms to renderpass encoder
        mesh.uniforms.refreshUniforms()
        encoder?.setVertexBuffer(mesh.uniforms.buffer.buffer!, offset: 0, index: 1)
        
        // 4. attach renderpass encoder with: a draw call command
        encoder?.drawIndexedPrimitives(type: .triangle, indexCount: mesh.indexCount, indexType: .uint16, indexBuffer: mesh.indexBuffer.buffer!, indexBufferOffset: 0)
